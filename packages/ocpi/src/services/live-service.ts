...
export function etaMinutes(samples: Array<{ soc: number; at: number }>): number | undefined {
...
export class LiveService {
  ...
  async getLive(tenantId: number, now: () => number = Date.now): Promise<LiveEvse[]> {
    const result = await this.ocpiGraphqlClient.request<LiveQueryResult, { tenantId: number }>(
